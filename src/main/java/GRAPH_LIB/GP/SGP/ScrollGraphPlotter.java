...
package GRAPH_LIB.GP.SGP;

import GRAPH_LIB.GP.*;
import GRAPH_LIB.GP.GPKeyEvent;
import GRAPH_LIB.GP.SGP.XYZ.XYZStepSeriesContour;
import GRAPH_LIB.GP.SGP.XYZ.XYZStepSeriesGraph;
...
//        int row=2;
    ...
    public void StepUpdated(SGPEvent e){
        ...
        for(int i=0;i<graphs.size();i++){
            ...
            }else if(graphs.get(i) instanceof XYZStepSeriesGraph){
                ((XYZStepSeriesGraph)graphs.get(i)).update(step);
            }else if(graphs.get(i) instanceof StepScrollGraph){
                ((StepScrollGraph)graphs.get(i)).update(step);
            }else if(graphs.get(i) instanceof XYZStepSeriesContour){
                ((XYZStepSeriesContour)graphs.get(i)).update(step);
            }
        }
