 ...
 * @author araki keita
 */
public class RevisedCloughModel extends AbstractModel{
    public double iniK;
    public double K;
    public double Qy;
    public double gamma;
     ...
     * @param mass      質量
     */
    public RevisedCloughModel(double K,double gamma,double Qy,double h,double mass){
        iniK=K;
        this.gamma=gamma;
        this.mass=mass;
        this.h=h;
    ...
    public void setK(double[] RD,double[] res,double Q){
        ...
        if(Q>=0){
            ...
            if(res[1]<0){
                K=iniK*Math.pow(Math.abs((RD[0]+Qy/iniK)/(Qy/iniK)), -0.4);
                PlasticFlag=false;
                Kflag=2;
            }
...
//                System.out.println(changeQ+"   "+changeDis);
            ...
            if(res[1]>0){
                K=iniK*Math.pow(Math.abs((RD[1]-Qy/iniK)/(Qy/iniK)), -0.4);
                PlasticFlag=false;
                Kflag=5;
            }
            else if(res[1]<=0&&Q>LinearLoad){
